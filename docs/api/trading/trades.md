...
```bash
curl -X 'GET' \
  'http://uat.quantxpress.com/interactive/v1/api/v1/order/trades' \
...
| status                 | string      | Current status of the order (New, Filled, Cancelled, etc.)       |
| orderQuantity         | integer     | Total quantity requested in the order                            |
...
| lastTradedQuantity   | integer     | Quantity executed in the most recent trade                       |
| lastTradedPrice      | number      | Price at which last trade was executed                           |
| cumulativeQuantity    | integer     | Total quantity executed so far                                   |
| leavesQuantity        | integer     | Remaining quantity yet to be executed                            |
| tif                    | string      | Time-in-force instruction (GFD, IOC, etc.)                       |
| orderExpiryDate      | integer     | Expiry timestamp of the order                                    |
| orderDisclosedQuantity| integer    | Quantity disclosed to the market                                 |
...
| orderModificationCount| integer    | Number of times the order was modified                           |
| orderTradeCount      | integer     | Number of trades executed for the order                          |
...
| sequenceNumber        | integer     | Sequence number for order tracking                               |